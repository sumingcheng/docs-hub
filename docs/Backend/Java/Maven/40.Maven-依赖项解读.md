...
**说明**
通过引入 `springfox-boot-starter`，我可以快速集成 Swagger 文档生成工具，无需复杂的配置。启动项目后，访问 `http://localhost:8080/swagger-ui/` 即可查看生成的 API 文档。
...
MySQL JDBC 驱动，允许 Java 程序连接到 MySQL 数据库。
...
**说明**
`ruoyi-generator` 通过配置数据库表信息，可以自动生成对应的实体类、Mapper 接口、Service 层以及控制器，大幅提升开发效率。
